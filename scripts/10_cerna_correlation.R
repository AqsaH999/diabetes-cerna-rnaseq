...
# map gene symbols -> Ensembl IDs (the vst matrix is keyed by Ensembl)
...
cat("lncRNA-mRNA pairs:", nrow(pairs),
    "| mappable to expression matrix:",
    sum(!is.na(pairs$lnc_id) & !is.na(pairs$mrna_id)), "\n")
pairs <- pairs[!is.na(lnc_id) & !is.na(mrna_id)]

# Spearman correlation across the 57 donors
rho <- numeric(nrow(pairs)); pv <- numeric(nrow(pairs))
for (i in seq_len(nrow(pairs))) {
  ct <- suppressWarnings(cor.test(vst[pairs$lnc_id[i], ],
                                  vst[pairs$mrna_id[i], ], method = "spearman"))
  rho[i] <- unname(ct$estimate); pv[i] <- ct$p.value
}
pairs[, `:=`(rho = rho, p = pv)]
pairs[, padj := p.adjust(p, method = "BH")]

kept <- pairs[rho > 0 & padj < 0.05]
fwrite(pairs, "results/tables/cerna_pair_correlation.csv")
fwrite(kept,  "results/tables/cerna_pairs_supported.csv")

cat("\nPositively correlated:", sum(pairs$rho > 0), "of", nrow(pairs), "\n")
cat("Positive AND padj < 0.05:", nrow(kept), "\n")
if (nrow(kept) > 0) cat("Median rho (kept):", round(median(kept$rho), 3), "\n\n")
print(head(kept[order(-rho)][, .(lncRNA, target_symbol,
                                 rho = round(rho, 3), padj = signif(padj, 3))], 15))

# rebuild the expression-supported network
tri_ok <- merge(triads, kept[, .(lncRNA, target_symbol)],
                by = c("lncRNA", "target_symbol"))
fwrite(tri_ok, "results/tables/cerna_triads_supported.csv")

cat("\nAxes retained:", nrow(tri_ok), "of", nrow(triads), "\n")
cat("lncRNAs:", uniqueN(tri_ok$lncRNA),
    "| miRNAs:", uniqueN(tri_ok$mature_mirna_id),
    "| mRNAs:",  uniqueN(tri_ok$target_symbol), "\n")
print(tri_ok[, .N, by = lncRNA][order(-N)])
